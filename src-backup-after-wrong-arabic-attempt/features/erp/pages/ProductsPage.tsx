...
export const ProductsPage = () => {
  ...
  const summaryCards = [
    { label: 'إجمالي المنتجات', value: formatNumber(mockProducts.length), tone: theme.palette.primary.main },
    { label: 'المنتجات النشطة', value: formatNumber(mockProducts.filter((product) => product.status === 'active').length), tone: theme.palette.success.main },
    { label: 'منتجات منخفضة المخزون', value: formatNumber(lowStockProducts.length), tone: theme.palette.warning.main },
    { label: 'قيمة المخزون', value: formatEgp(totalInventoryValue), tone: theme.palette.primary.main }
  ];

  return (
    <ErpFullWidthPage>
      <ErpPageHeader title="المنتجات" subtitle="إدارة كتالوج مستلزمات Megawatt الكهربائية" />
      ...
      <MainCard border elevation={0} contentSX={{ p: 0, '&:last-child': { pb: 0 } }} sx={{ width: '100%', maxWidth: '100%' }}>
        ...
        >
          ...
          <FormControl size="small" sx={{ minWidth: { xs: '100%', md: 190 } }}>
            <InputLabel id="product-category-filter-label">الفئة</InputLabel>
            <Select
          ...
          <FormControl size="small" sx={{ minWidth: { xs: '100%', md: 170 } }}>
            <InputLabel id="product-status-filter-label">الحالة</InputLabel>
            <Select
                ...
                <TableRow key={product.id} hover>
                  ...
                  <TableCell>
                    ...
                    <Typography variant="caption" color="text.secondary">
                      المخزون {formatNumber(product.stockQuantity)} / حد الطلب {formatNumber(product.reorderLevel)}
                    </Typography>
                  </TableCell>
